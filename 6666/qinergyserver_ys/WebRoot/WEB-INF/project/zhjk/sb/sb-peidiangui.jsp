...
<script type="text/javascript">
	...
	var gourl = jumpPageUrl +  "commens/zhjk/sb/peidiangui.htm";
 	...
 	/*交流配电柜*/
 	function getJiaoLiuPdData(param,stat){
 		$.ajax({
	    	url:url+"getAcdbInfByEquNum.htm",
	    	data : param,
	    	dataType:"json",
	    	type:"post",
	    	success:function(data){
	    		if (data.resultcode=="USR000") {
	    			  if(data.data.length > 0 ){
	    				  if(data.data[0].acdbEquLst.length > 0 ){
	    					  if(stat === "" || stat == undefined || stat == null ){
	    						  /*显示各个状态的数量*/
	    					  ...
	    					  /*表格数据 */
	    					  var html = '';
	    					  for(var i=0;i<data.data[0].acdbEquLst.length;i++){
	    						  if(i < (currentPage*everyPageCount)){
		    						  var bms = data.data[0].acdbEquLst[i];
		    						  var sta ="";
		    						  var cls ="";
		    						  if(bms.stat == 0){
		    							  sta = "通讯中断";
		    						  ...
		    						  var stat1Dw = bms.stat1 == 0 ? '断开' :   bms.stat1 == 1 ?  '闭合' : '--' ;
		    						  var stat2Dw = bms.stat2 == 0 ? '断开' :   bms.stat2 == 1 ?  '闭合' : '--' ;
		    						  var stat3Dw = bms.stat3 == 0 ? '断开' :   bms.stat3 == 1 ?  '闭合' : '--' ;
		    						  var stat4Dw = bms.stat4 == 0 ? '断开' :   bms.stat4 == 1 ?  '闭合' : '--' ;
		    						  var stat5Dw = bms.stat5 == 0 ? '断开' :   bms.stat5 == 1 ?  '闭合' : '--' ;
		    				  ...
		    				  pageData = data.data[0].acdbEquLst;
		    				  //算出总页数
		    				  countAll = 0 ;
