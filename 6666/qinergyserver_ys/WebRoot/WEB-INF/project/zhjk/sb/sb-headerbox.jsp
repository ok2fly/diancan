...
<script type="text/javascript">
	...
	//跳转页面
	var gourl = jumpPageUrl+"commens/zhjk/sb/headerbox.htm";
	    		...
	    		if (data.resultcode=="USR000") {
	    			  if(data.data.length > 0 ){
	    				  if(data.data[0].boxEquLst ){
	    					  ...
	    					  /*表格数据 */
	    					  var html = '';
	    					  for(var i=0;i<data.data[0].boxEquLst.length;i++){
	    						  if(i < (currentPage*everyPageCount)){
		    						  var pvs = data.data[0].boxEquLst[i];
		    						  var sta ="";
		    						  var cls ="";
		    						  if(pvs.stat == 0){
		    							  sta = "通讯中断";
		    						  ...
		    						  html +='<div class="lie1">'+
		    						  			'<div class="sb-sbh bgff bor-radius" onclick="jumpPage('+pvs.id+',\''+pvs.equ_num+'\')">'+
					    							...
					    							'<div class="img-box">'+
					    							'<img src="'+jumpPageUrl+'img/sb/sb-hl.png">'+
					    							...
					    							'<div class="a4-right2 bortopccc">'+
					    								'<div>'+getFixedNum(pvs.udc)+'</div><p class="dw">V</p>'+
					    								'<div>'+getFixedNum(pvs.idc)+'</div><p class="dw">A</p>'+
					    								'<div>'+getFixedNum(pvs.pdc)+'</div><p class="dw">kW</p>'+
					    								'<div>'+getFixedNum(pvs.temp)+'</div><p class="dw">°C</p>'+
					    								'<div>'+getFixedNum(pvs.disRate)+'</div><p class="dw">%</p>'+
					    							'</div>'+
		    									'</div>'+
		    								'</div>';
	    						  }
		    				  ...
		    				  pageData = data.data[0].boxEquLst;
		    				  //算出总页数
		    				  countAll = 0 ;
