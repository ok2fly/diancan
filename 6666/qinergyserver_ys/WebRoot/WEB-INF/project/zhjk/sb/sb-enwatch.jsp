...
<script type="text/javascript">
	...
	//跳转页面
	var gourl = jumpPageUrl+"commens/zhjk/sb/enwatch.htm";
	    	...
	    	success:function(data){
	    		if (data.resultcode=="USR000") {
	    			  if(data.data.length > 0 ){
	    				  if(data.data[0].meterEquLst.length > 0 ){
	    					  if(stat === "" || stat == undefined || stat == null ){
	    						  /*显示各个状态的数量*/
	    					  ...
	    					  /*表格数据 */
	    					  var html = '';
	    					  for(var i=0;i<data.data[0].meterEquLst.length;i++){
	    						  if(i < (currentPage*everyPageCount)){
		    						  var bms = data.data[0].meterEquLst[i];
		    						  var sta ="";
		    						  var cls ="";
		    						  if(bms.stat == 0){
		    							  sta = "通讯中断";
		    						  ...
		    						  html +='<div class="lie1">'+
		    						  			'<div class="sb-sbh bgff bor-radius" onclick="jumpPage('+bms.id+',\''+bms.equ_num+'\')" >'+
					    							...
					    							'<div class="img-box">'+
					    							'<img src="'+jumpPageUrl+'img/sb/sb-db.png">'+
					    							...
					    							'<div class="a4-right2 bortopccc">'+
					    								'<div>'+getFixedNum(bms.psum)+'</div><p class="dw">kW</p>'+
					    								'<div>'+getFixedNum(bms.phi)+'</div><p class="dw">kWh</p>'+
					    								'<div>'+getFixedNum(bms.phe)+'</div><p class="dw">kVarh</p>'+
					    								'<div>'+getFixedNum(bms.qhi)+'</div><p class="dw">kWh</p>'+
					    								'<div>'+getFixedNum(bms.qhe)+'</div><p class="dw">kVarh</p>'+
					    							'</div>'+
		    									'</div>'+
		    								'</div>';
	    						  }
		    				  ...
		    				  pageData = data.data[0].meterEquLst;
		    				  //算出总页数
		    				  countAll = 0 ;
