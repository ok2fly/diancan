	  		...
	  		<div class="bg-ff pl15 pt10 clr pl8">
		        <div class="query">
		         ...
		         <div class="query pt10"> 
			          <div class="fr mr15">
			            <a class="queryicon layer XZJXJH_QX"  data-url="<%=basePath%>/commens/jhgl/jxjh/add.htm" data-title="新增计划"  ><i class="add"></i>添加检修计划</a>
        				 ...
        				 for(var i = 0; i< data.data.length ; i++){
    		    				...
    		    				htmls += "<tr>"+
    		    					...
    		    					"<td>"+getIsNull(d.tour_plan_name)+"</td>"+
    				              	...
    				              	"<td><a  class='layer XGJXJH_QX' data-url='<%=basePath%>commens/jhgl/jxjh/edit.htm?id="+d.id+"' data-title='修改'    href='javascript:void(0);' >修改 </a>"+
    				              	"<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a  href='javascript:void(0);' class='SCJXJH_QX' onclick='del("+d.id+")'>删除</a>"+ss+
    				              	"</tr>";
...
function excute(id){
	layer.confirm("确定开始执行？",function(){
		$.ajax({
	    	url:url+"updOverhaulPlanRunFlat.htm",
	    	data : "id="+id,
	    	dataType:"json",
	    	type:"post",
	    	success:function(data){
...
function download1(){
	...
	var  layerC = layer.confirm("确定下载？",function(){
		...
		$.ajax({
			url : url + "OverhaulPlanExport.htm",
			dataType : "json",
			type : "post",
			success : function(data) {
				if (data.resultcode && data.resultcode != "USER000") {
					...
					location.href = url + "OverhaulPlanExport.htm";
				}
				layer.close(layerC)
			},
			error : function(){
