...
<div class="pt18">
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="poptable">
		...
		<tr>
			<th>计划充电量(kWh)</th>
			<td><input type="number" id="plan_phi"   placeholder="请输入计划充电量" class="popinput quinputcol requiredInp" oninput="if(value.length>11)value=value.slice(0,11)"></td>
			<th>计划放电量(kWh)</th>
			<td><input type="number" id="plan_phe"   placeholder="请输入计划放电量" class="popinput quinputcol requiredInp" oninput="if(value.length>11)value=value.slice(0,11)" ></td>
		</tr>
	</table>
...
function insert(){
	...
	var crt_use_id=$("#userId").val();
	 ...
	 if(!isNull(plan_phi)){
			param  += "&plan_phi="+plan_phi;
		}
	 var  plan_phe = $("#plan_phe").val();
	 if(!isNull(plan_phe)){
			param  += "&plan_phe="+plan_phe;
		}
