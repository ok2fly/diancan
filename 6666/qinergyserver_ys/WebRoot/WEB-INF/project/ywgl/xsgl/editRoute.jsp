	...
	<input type="hidden"  value="${routeId}"  id="routeId" name="routeId">
 	<table cellpadding="0"   border="0" width="100%" class="poptable">
		...
		<tr>
			...
			<th>附件</th>
			<td><input type="file" name="files"  placeholder="请选择附件" id="files" class="popinput requiredInp"></td>
		</tr>
		<tr>
			<th>电站</th>
			<td>
...
function getById(treId){
	
	 $.ajax({
	    	url:url+"selTourRecordByRouteId.htm",
	    	data : "treId="+treId,
	    	dataType:"json",
	    	async:false,
	    	type:"post",
...
function uploadImg(){
	...
	//必填项的校验 
	 ...
	 layer.confirm("确定保存？",function(){
        ...
        if(filesize < maxSize){  
			var option = {
		     　　 	url : url+'updOrInsTourRecord.htm',
		    　　  	type : 'POST',
		     　　 	dataType : 'json',
		      　　	contentType: false,
		       	processData: false,
