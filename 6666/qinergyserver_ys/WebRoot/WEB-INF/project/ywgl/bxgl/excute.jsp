...
<div class="pt18" style="height:100%;">
	...
	<div  class="pl8 pr8 pb18 bg-ff">
		<div class="clr wauto" >
		 	 <a  class='layer fr' style="padding:5px;border:1px solid #ccc;border-radius:3px;background-color: #00b8b7;color:#fff;" data-url='<%=basePath%>commens/ywgl/bxgl/editRoute.htm?task_id=${id}' data-title='新增记录' data-width='650px' data-height='300px'   href='javascript:void(0);' >新增记录 </a> 
	     </div>
     </div>
...
function update(){
	...
	var param =  "id="+id+"&check_use_id="+task_check_id ;
	layer.confirm("确定修改？",function(){
		$.ajax({
	    	url:url+"updRepairStaFour.htm",
	    	data : param ,
	    	dataType:"json",
	    	type:"post",
	    	success:function(data){
	    		if (data.resultcode=="USR000") {
	            	...
	            	$(".cancle").click();
	   	    		window.parent.layer.alert("提交审核成功");
	   	    		layer.closeAll();
				} else {
	               layer.alert(data.desc);
				}
