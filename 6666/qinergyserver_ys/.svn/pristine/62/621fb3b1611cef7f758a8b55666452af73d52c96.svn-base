...
<script type="text/javascript" src="<%=basePath%>js/exportExcel.js?pubVersion=201802070001"></script>
	...
	function getDataDown(){
		...
		layer.confirm("确定下载？",function(){
			...
			$.ajax({
				url:url+"getPCInfStaBetweenDay.htm",
				data:param,
				type:"post",
				dataType:"json",
				success:function(data){
						...
						if( data.data.length > 0 ){
							...
							htmls +=  
									'<tr>'+
										'<td rowspan="4">统计</td>'+
										'<td>最大</td>'+
										'<td>'+getFixedNum5(d.max_phi)+'</td>'+
										'<td>'+getFixedNum5(d.max_cnv_eff)+'</td>'+
										'<td>'+getFixedNum5(d.max_charge_time)+'</td>'+
									...
									'<tr>'+
										'<td>最小</td>'+
										'<td>'+getFixedNum5(d.min_phi)+'</td>'+
										'<td>'+getFixedNum5(d.min_cnv_eff)+'</td>'+
										'<td>'+getFixedNum5(d.min_charge_time)+'</td>'+
									...
									'<tr>'+
										'<td>平均</td>'+
										'<td>'+getFixedNum5(d.avg_phi)+'</td>'+
										'<td>'+getFixedNum5(d.avg_cnv_eff)+'</td>'+
										'<td>'+getFixedNum5(d.avg_charge_time)+'</td>'+
									...
									'<tr>'+
										'<td>合计</td>'+
										'<td>'+getFixedNum5(d.sum_phi)+'</td>'+
										'<td>-</td>'+
										'<td>'+getFixedNum5(d.sum_charge_time)+'</td>'+
							  ...
							  for(var i=0;i<d.pcLst.length;i++){
								  ...
								  var trColor7 = "";
								  if(s!=null){
									  if(s.color_i!=null && s.color_i!= undefined ){
										  trColor  = s.color_i == 1 ? "background:red ;color:#fff;  " :s.color_i == 2 ? "background:blue ;color:#fff;" : s.color_i == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  ...
									  if(s.color_ct!=null&& s.color_ct!= undefined ){
										  trColor3  = s.color_ct == 1 ? "background:red ;color:#fff;  " :s.color_ct == 2 ? "background:blue ;color:#fff;" : s.color_ct == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  ...
									  htmls += '<tr  >'+
													'<td colspan="2" style="mso-number-format:\'\@\';">'+getIsNull2(s.equ_nam)+'</td>'+
													'<td style="'+trColor+'">'+getFixedNum5(s.phi)+'</td>'+
													'<td style="'+trColor2+'">'+getFixedNum5(s.cnv_eff)+'</td>'+
													'<td style="'+trColor3+'">'+getFixedNum5(s.charge_time)+'</td>'+
							  ...
							  downLoadFile('datalist3',"储能逆变器报表");
						}else{
							$("#datalist2").empty();
							layer.close(indexlayer);
							layer.alert("暂无数据");
				...
				if(data.resultcode=="USR000"){
					...
					if( data.data.length > 0 ){
						...
						htmls +=  
						  ...
						  for(var i=0;i<d.pcLst.length;i++){
							  ...
							  if(i < (currentPage*everyPageCount)){
								  ...
								  if(s!=null){
									  ...
									  htmls += '<tr >'+
													'<td colspan="2">'+getIsNull2(s.equ_nam)+'</td>'+
													'<td style="'+trColor+'">'+getFixedNum5(s.phi)+'</td>'+
													'<td style="'+trColor2+'">'+getFixedNum5(s.cnv_eff)+'</td>'+
													'<td style="'+trColor3+'">'+getFixedNum5(s.charge_time)+'</td>'+
													'<td style="'+trColor4+'">'+getFixedNum5(s.good_run_time)+'</td>'+
	    				  ...
	    				  //算出总页数
	    				  countAll = 0 ;
    					  if(getPageData[0].pcLst.length/everyPageCount > parseInt(getPageData[0].pcLst.length/everyPageCount)){   
    						  countAll=parseInt(getPageData[0].pcLst.length/everyPageCount)+1;   
    				      }else{   
    				    	  countAll=parseInt(getPageData[0].pcLst.length/everyPageCount);   
    				       };
	    				  //countAll = data.totalPage;
	...
	function getPageData1(){
		 ...
		 var max = 0 ;
		 if ( (currentPage*everyPageCount) >  getPageData[0].pcLst.length-1 ){
			 max = getPageData[0].pcLst.length; 
		 }else{
			 max = (currentPage*everyPageCount) ;
		 }
		 
		 for(var i=((currentPage-1)*everyPageCount);i<max;i++){
			  var s =getPageData[0].pcLst[i];
			  if(i < (currentPage*everyPageCount)){
				  var trColor = "";
				  var trColor2 = "";
				  var trColor3 = "";
