  ...
  <div class=viewFramework> 
     ...
     <div class="bg-ff pb18 pl15 pt10 clr pl8">
        <div class="query">
	          ...
	          <div class="fr mr15">
	            <a class="queryicon  xzzz XJYZ_QX"  data-url="<%=basePath%>/commens/xtgl/yzgl/add.htm?id=0&level=1" data-title="添加业主"  onclick="addOrg(this)" ><i class="add"></i>添加业主</a>
	          </div>
        </div>
      </div>
...
function getCom(){
	...
	$.ajax({                
		...
		data : "id="+userId+"&com_rol=1&com_lev="+checkLevel+"&com_fat_id="+checkId,
...
function getXj(id){
	...
	if(id == '0'){
		...
		$(".xzzz").attr("data-url","<%=basePath%>commens/xtgl/yzgl/add.htm?id=0&level=1");
		checkId = 0 ;
		checkLevel = 1 ;
		getCom();
		return;
	...
	$.ajax({                
		...
		success: function(data){                          
			if('USR000' == data.resultcode){
				...
				$(".xzzz").attr("data-url","<%=basePath%>commens/xtgl/yzgl/add.htm?id="+id+"&level=2");
		...
		$.ajax({                
			...
			success: function(data){                          
				if('USR000' == data.resultcode){
					...
					$(".xzzz").attr("data-url","<%=basePath%>commens/xtgl/yzgl/add.htm?id="+id+"&level=3");
				}else{
					layer.alert('数据解析错误');
				}
			...
			if(i<(currentPage1*everyPageCount1)){
				html  +=   "<tr>"+        
	              ...
	              "<td>"+basSci.pro_nam+basSci.cit_nam+basSci.are_nam+basSci.com_add+"</td>"+
        		   "<td><a href='#' target='_self' class='layer ZZXGGS' data-title='组织机构修改'  data-url='"+jumpPageUrl+"commens/xtgl/yzgl/edit.htm?id="+basSci.id+"' >修改</a>"+
   		 	   "<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href='#' target='_self' class='layer ZZXGGS' data-title='组织机构修改'  data-url='"+jumpPageUrl+"commens/xtgl/yzgl/detal.htm?id="+basSci.id+"' >详细</a>"+ 
        		 "<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href='#' target='_self' class='layer ZZSCTB' data-title='上传图标' data-height='200px' data-url='"+jumpPageUrl+"commens/xtgl/yzgl/addlogo.htm?id="+basSci.id+"' >上传图标</a>"+	   
	...
	var max = 0 ;
	 ...
	 for(var i=((currentPage1-1)*everyPageCount1);i<max;i++){
         ...
         "<td>"+basSci.pro_nam+basSci.cit_nam+basSci.are_nam+basSci.com_add+"</td>"+
		   "<td><a href='#' target='_self' class='layer ZZXGGS' data-title='组织机构修改'  data-url='"+jumpPageUrl+"commens/xtgl/yzgl/edit.htm?id="+basSci.id+"' >修改</a>"+
	 	   "<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a>详细</a>"+ 
		 "<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a href='#' target='_self' class='layer ZSCTB' data-title='上传图标' data-height='200px' data-url='"+jumpPageUrl+"commens/xtgl/yzgl/addlogo.htm?id="+basSci.id+"' >上传图标</a>"+	   
	 	   "<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a  class='ZZSCGS' href='#'id='"+basSci.id+"' onclick=delThis(this) target='_self'>删除</a>"+ 
	 	    "</td>"+
	 	    "</tr>";
	 }
