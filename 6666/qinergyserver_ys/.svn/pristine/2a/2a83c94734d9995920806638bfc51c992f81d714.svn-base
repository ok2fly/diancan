//统计分析的电站分析公司联动的文件、查看是否有缓存，如没有 就查询第一个
...
function setPws(val){
	if(val == 'qxz'){
		...
		if(sessionStorage.getItem("pwsId") && sessionStorage.getItem("pwsId") != undefined){
			getTjfxData();//每个页面都加入这个方法
		}else{
			if(firstVal != '' ){
				getTjfxData();//每个页面都加入这个方法
			} 
