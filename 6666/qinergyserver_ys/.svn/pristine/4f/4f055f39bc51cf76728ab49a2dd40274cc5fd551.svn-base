  ...
  <div class="rc-body">
		...
		<div class="rc-right">
	     <div class="bg-ff pb18 pl15 pt10 clr pl8">
	        <div class="query">
	          <div class="fl">
	          	<input type="text" value="" id="info" placeholder="请输入名称" class="fl quinput">
	          	<input type="button" value="查询" class="querybut">
	          </div>
	          <div class="fr mr15">
	            <a class="queryicon TJAQXX_QX"  onclick="add()" data-title="添加安全信息"  ><i class="add"></i>添加安全信息</a>
	          	<a class="queryicon layer XJLX_QX"  data-url="<%=basePath%>commens/aqgl/addType.htm" data-title="新建类型"  data-height="200px"  ><i class="add"></i>新建类型</a>
...
<script type="text/javascript">
	    		...
	    		if (data.resultcode=="USR000") {
	    			 ...
	    			 if(data.data.length > 0 ){
	    				 ...
	    				 for(var i = 0; i< data.data.length ; i++){
			    				var d =  data.data[i];
			    				htmls +=  "<li><span class='file' onclick='checkTyp("+d.id+")'>"+d.sct_typ_nam+"</span></li>";
		    			 }
	    				 htmls +=  "</ul>";
		    			 htmls += "</li>";
	    			 }else{
	    				 ...
	    				 for(var i = 0; i< data.data.length ; i++){
			    				...
			    				htmls += "<tr>"+
			    					...
			    					"<td>"+((currentPage-1)*25+i+1)+"</td>"+
					              	"<td>"+getIsNull(d.sct_nam)+"</td>"+
					              	"<td>"+getIsNull(d.sct_dec)+"</td>"+
					              	"<td>"+getIsNull(d.use_nam)+"</td>"+
					              	"<td>"+getLocalDateAndTime(d.crt_tim,1)+"</td>"+
					              	"<td><a  class='layer CKWJ_QX'  data-url='<%=basePath%>commens/aqgl/query.htm?id="+d.id+"' data-title='查看文件'   href='javascript:void(0);' >查看文件 </a>"+
					              	"<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a  class='layer XGAQXX_QX'  data-url='<%=basePath%>commens/aqgl/edit.htm?id="+d.id+"' data-title='修改' data-height='300px'  href='javascript:void(0);' >修改 </a>"+
					              	"<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a  href='javascript:void(0);' class='SCAQXX_QX' onclick='del("+d.id+")'>删除</a>"+
					              	"<span>&nbsp;&nbsp;|&nbsp;&nbsp;</span><a  class='layer SCWJ_QX'  data-url='<%=basePath%>commens/aqgl/editFile.htm?id="+d.id+"' data-title='上传文件' data-height='300px'  href='javascript:void(0);' >上传文件</a></td>"+
					              	"</tr>";
		    			 }
	...
	function editType(){
		if(checkId == "" || checkId == 'qb' ){
			...
			layer.open({
				  ...
				  shadeClose: false,
				  area: ['770px','450px'],
				  content: "<%=basePath%>commens/aqgl/editType.htm?id="+checkId   //iframe的url
			});
	...
	function add(){
		...
		layer.open({
			  ...
			  shadeClose: false,
			  area: ['770px','450px'],
			  content: "<%=basePath%>commens/aqgl/add.htm?type="+type  //iframe的url
		});
	}
	</script>
</body>
