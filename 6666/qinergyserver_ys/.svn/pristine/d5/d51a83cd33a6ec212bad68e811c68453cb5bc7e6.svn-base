 	...
 	$.ajax({
	    	...
	    	success:function(data2){
	    		if (data2.resultcode=="USR000") {
	    			 var htmls = "";
   					var d =  data2.data;
    				$("#par_ide").val(d.par_ide);
    				$("#par_nam").val(d.par_nam);
    				$("#par_val").val(d.par_val);
    				$("#sor_num").val(d.sor_num);
    				getType(d.dic_typ_id);
				} else {
	    		...
	    		if (data2.resultcode=="USR000") {
	    			 ...
	    			 if(data2.data.length > 0 ){
	    				 for(var i = 0; i< data2.data.length ; i++){
		    				var d =  data2.data[i];
		    				if(id == d.id){
		    					htmls +=  "<option value='"+d.id+"' selected>"+d.dic_typ_nam +"</option>";
		    				}else{
...
function update(){
	...
	var reFlag = false ; 
	 ...
	 layer.confirm("是否修改?",function(){
		...
		$.ajax({
	    	url:url+"updDataDic.htm",
	    	data:"id="+id+"&par_ide="+par_ide+"&par_nam="+par_nam+"&par_val="+par_val+"&sor_num="+sor_num+"&dic_typ_id="+dic_typ_id,
	    	dataType:"json",
	    	type:"post",
