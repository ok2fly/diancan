...
<script type="text/javascript">
	...
	//跳转页面
	var gourl = jumpPageUrl+"commens/zhjk/sb/jielie.htm";;
 	$(function (){
 	...
 	/*解列*/
 	function getJieLieData(param,stat){
 		$.ajax({
	    	url:url+"getDctdevInfLst.htm",
	    	data : param,
	    	dataType:"json",
	    	type:"post",
	    	success:function(data){
	    		if (data.resultcode=="USR000") {
	    			  if(data.data.length > 0 ){
	    				  if(data.data[0].dctdevEquLst.length > 0 ){
	    					  if(stat === "" || stat == undefined || stat == null ){
	    						  /*显示各个状态的数量*/
		    					  $(".brgreena").html(getIsZero(data.data[0].norStatCount));
		    					  $(".bryela").html(getIsZero(data.data[0].downStatCount));
		    					  $(".brblacka").html(getIsZero(data.data[0].alaStatCount));
		    					  $(".brgraya").html(getIsZero(data.data[0].commStatCount));
	    				 	  }
	    					  
	    					  /*表格数据 */
	    					  var html = '';
	    					  for(var i=0;i<data.data[0].dctdevEquLst.length;i++){
	    						  if(i < (currentPage*everyPageCount)){
		    						  var pcs = data.data[0].dctdevEquLst[i];
		    						  var sta ="";
		    						  var cls ="";
		    						  if(pcs.stat == 0){
		    							  sta = "通讯中断";
		    						  ...
		    						  html +='<div class="lie1">'+
		    						  			'<div class="sb-sbh bgff bor-radius" onclick="jumpPage('+pcs.id+',\''+pcs.equ_num+'\')">'+
					    							...
					    							'<div class="img-box">'+
					    							'<img src="'+jumpPageUrl+'img/sb/sb-jiel.png">'+
					    							...
					    							'<div class="a4-right2 bortopccc">'+
					    								'<div>'+getFixedNum(pcs.ua1)+'</div><p class="dw">V</p>'+
					    								'<div>'+getFixedNum(pcs.ua2)+'</div><p class="dw">V</p>'+
					    								'<div>'+getFixedNum(pcs.freq1)+'</div><p class="dw">Hz</p>'+
					    								'<div>'+getFixedNum(pcs.freq2)+'</div><p class="dw">Hz</p>'+
					    							'</div>'+
		    									'</div>'+
		    								'</div>';
	    						  }
	    					  ...
	    					  pageData = data.data[0].dctdevEquLst;
	    					  //算出总页数
		    				  countAll = 0 ;
