...
<form  action="" id="uploadModel"  enctype="multipart/form-data" >
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="poptable">
		<tr>
			<th>文档类型</th> 
			<td><select class="popinput requiredInp" placeholder="请选择文档类型"    id="typeId" name="file_type" ></select></td>
			<th>文件</th> 
			<td><input type="file" placeholder="请选择文件"  name="file"  id="file" class="popinput quinputcol requiredInp"></td>
		</tr>
		<tr>
			<th>备注</th>
			<td colspan="3">
	    		...
	    		if (data.resultcode=="USR000") {
	    			 ...
	    			 if(data.data.length > 0 ){
	    				 for(var i = 0; i< data.data.length ; i++){
			    				var d =  data.data[i];
			    				 htmls +=  "<option value='"+d.id+"'>"+d.fil_typ_nam+"</option>";
		    			 }
	    			 } 
...
function insert(){
	...
	var  remark = parseInt($("#remark").val());
