...
<script type="text/javascript" src="<%=basePath%>js/exportExcel.js?pubVersion=201802070001"></script>
	...
	function getDataDown(){
		...
		layer.confirm("确定下载？",function(){
			...
			$.ajax({
				url:url+"getChpInfStaBetweenDay.htm",
				data:param,
				type:"post",
				dataType:"json",
				success:function(data){
						...
						if( data.data.length > 0 ){
							...
							htmls +=  
									'<tr>'+
										'<td rowspan="4">统计</td>'+
										'<td>最大</td>'+
										'<td>'+getFixedNum5(d.max_cha_vol)+'</td>'+
										'<td>'+getFixedNum5(d.max_cha_vol_whe_equ)+'</td>'+
										'<td>'+getFixedNum5(d.max_cha_vol_tim)+'</td>'+
									...
									'<tr>'+
										'<td>最小</td>'+
										'<td>'+getFixedNum5(d.min_cha_vol)+'</td>'+
										'<td>'+getFixedNum5(d.min_cha_vol_whe_equ)+'</td>'+
										'<td>'+getFixedNum5(d.min_cha_vol_tim)+'</td>'+
									...
									'<tr>'+
										'<td>平均</td>'+
										'<td>'+getFixedNum5(d.avg_cha_vol)+'</td>'+
										'<td>'+getFixedNum5(d.avg_cha_vol_whe_equ)+'</td>'+
										'<td>'+getFixedNum5(d.avg_cha_vol_tim)+'</td>'+
									...
									'<tr>'+
										'<td>合计</td>'+
										'<td>'+getFixedNum5(d.sum_cha_vol)+'</td>'+
										'<td>'+getFixedNum5(d.sum_cha_vol_whe_equ)+'</td>'+
										'<td>'+getFixedNum5(d.sum_cha_vol_tim)+'</td>'+
							  ...
							  for(var i=0;i<d.chpLst.length;i++){
								  var s = d.chpLst[i];
								  ...
								  if(s!=null){
									  ...
									  if(s.color_we!=null&& s.color_we!= undefined ){
										  trColor2  = s.color_we == 1 ? "background:red ;color:#fff;  " :s.color_we == 2 ? "background:blue ;color:#fff;" : s.color_we == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  if(s.color_cvt!=null&& s.color_cvt!= undefined ){
										  trColor3  = s.color_cvt == 1 ? "background:red ;color:#fff;  " :s.color_cvt == 2 ? "background:blue ;color:#fff;" : s.color_cvt == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  if(s.color_grt!=null&& s.color_grt!= undefined ){
										  trColor4  = s.color_grt == 1 ? "background:red ;color:#fff;  " :s.color_grt == 2 ? "background:blue ;color:#fff;" : s.color_grt == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  if(s.color_ft!=null&& s.color_ft!= undefined ){
										  trColor5  = s.color_ft == 1 ? "background:red ;color:#fff;  " :s.color_ft == 2 ? "background:blue ;color:#fff;" : s.color_ft == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  if(s.color_flt!=null&& s.color_flt!= undefined ){
										  trColor6  = s.color_flt == 1 ? "background:red ;color:#fff;  " :s.color_flt == 2 ? "background:blue ;color:#fff;" : s.color_flt == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  if(s.color_cit!=null&& s.color_cit!= undefined ){
										  trColor7  = s.color_cit == 1 ? "background:red ;color:#fff;  " :s.color_cit == 2 ? "background:blue ;color:#fff;" : s.color_cit == 3 ? "background:#E8C709 ;color:#fff;"  : "" ;
									  }
									  htmls += '<tr  >'+
													'<td colspan="2"  style="mso-number-format:\'\@\';">'+s.equ_nam+'</td>'+
													'<td style="'+trColor+'">'+getFixedNum5(s.cha_vol)+'</td>'+
													'<td style="'+trColor2+'">'+getFixedNum5(s.cha_vol_whe_equ)+'</td>'+
													'<td style="'+trColor3+'">'+getFixedNum5(s.cha_vol_tim)+'</td>'+
													'<td style="'+trColor4+'">'+getFixedNum5(s.good_run_time)+'</td>'+
													'<td style="'+trColor5+'">'+getFixedNum5(s.fault_time)+'</td>'+
													'<td style="'+trColor6+'">'+getFixedNum5(s.failure_time)+'</td>'+
													'<td style="'+trColor7+'">'+getFixedNum5(s.com_int_time)+'</td>'+
												'</tr>';
								  }
							  }
				...
				if(data.resultcode=="USR000"){
					...
					if( data.data.length > 0 ){
						...
						htmls +=  
	    				  ...
	    				  //算出总页数
	    				  countAll = 0 ;
    					  if(getPageData[0].chpLst.length/everyPageCount > parseInt(getPageData[0].chpLst.length/everyPageCount)){   
    						  countAll=parseInt(getPageData[0].chpLst.length/everyPageCount)+1;   
    				      }else{   
    				    	  countAll=parseInt(getPageData[0].chpLst.length/everyPageCount);   
    				      };
	    				  //countAll = data.totalPage;
	...
	function getPageData1(){
		 ...
		 var max = 0 ;
		 if ( (currentPage*everyPageCount) >  getPageData[0].chpLst.length-1 ){
			 max = getPageData[0].chpLst.length; 
		 }else{
			 max = (currentPage*everyPageCount) ;
		 }
		 
		 for(var i=((currentPage-1)*everyPageCount);i<max;i++){
			  var s =getPageData[0].chpLst[i];
			  if(i < (currentPage*everyPageCount)){
