...
<div class="pt18">
	...
	<table cellpadding="0"   border="0" width="100%" class="poptable">
		<tr>
			<th>巡视路线</th>
			<td><select class="popinput requiredInp" placeholder="请选择路线" id="tour_route_id"></select></td>
			<th>计划名称</th>
			<td><input type="text" placeholder="请输入计划名称"  id="tour_plan_name" class="popinput quinputcol requiredInp"></td>
		</tr>
		<tr>
			<th>计划开始日期</th>
			<td><input type="text" placeholder="请输入计划开始日期"  id="first_time" class="popinput quinputcol Wdate requiredInp" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'end_time\')}',minDate:'%y-%M-%d' })"></td>
			<th>计划结束时间</th>
		...
		<tr>
			<th>巡视周期规则</th>
			<td>
				<select placeholder="请选择巡视周期规则" id="day_type"  class="popinput requiredInp">
					<option value="2">按日</option>
					<option value="1">按月</option>
				</select>
			</td>
			<th>巡视周期</th>
			<td><input type="number" placeholder="请输入巡视周期" id="frequency"  class="popinput quinputcol requiredInp"  oninput="if(value.length>11)value=value.slice(0,11)"></td>
		</tr>
		<tr>
			<th>巡视工期(日)</th>
			<td><input type="number" placeholder="请输入巡视工期" id="plan_tour_time"  class="popinput quinputcol requiredInp" oninput="if(value.length>11)value=value.slice(0,11)"></td>
		</tr>
		<tr>
			<th>备注</th>
			<td colspan="3">
...
function insert(){
	...
	layer.confirm("确定保存？",function(){
		...
		$.ajax({
	    	url:url+"insertOrUpdTourPlan.htm",
	    	data : param,
	    	dataType:"json",
	    	type:"post",
	    	success:function(data){
