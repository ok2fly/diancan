...
<body class="pl8t0 bgfc">
		...
		<div class="dc-center">
			...
			<%-- <div class="ene-table bor-radius">
				<div class="a8">
					<div class="sto-box1">
						<div class="a4"><p>本次累计充电费(元)</p></div>						
					...
					<div class="sto-box1">
						<div class="a4"><p>本次累计充电量(kWh)</p></div>						
						<img src="<%=basePath %>img/nbq1.png" >
						<div class="sto-box2">
						<h2 id="tolPower">0.00</h2>
						</div>
					</div>
				</div>
			</div> --%>
...
function getData(equNum){
	$.ajax({
		url:url+"getIscsAcchpStdInfByEquNumTopOne.htm",
		type:"post",
		data:"equ_num="+equNum,
		//data:"equ_num=SHDL20170108",
		dataType:"json",
		async:false,
				...
				if(data.data.length > 0){
					...
					if(d != undefined){
						...
						if(data.data.length>0){
							...
							$("#cumMoney").html(getFixedNum(getFixedNum(d.cumMoney)/100));
	...
	$.ajax({
		url:url+"getIscsAcchpRelInfByEquNumTopOne.htm",
		type:"post",
		...
		success:function(data){
			if(data.resultcode=="USR000"){
				...
				$("#ua").html(getFixedNum(d.ua));
				$("#ia").html(getFixedNum(d.ia));
				$("#power").html(getFixedNum(d.power));
				$("#time").html(getFixedNum(getFixedNum(d.time)/3600));
				$("#money").html(getFixedNum(getFixedNum(d.money)/100));
				$("#tolPower").html(getFixedNum(d.tolPower));
				}
			}else{
				layer.alert(data.desc);
			}
	...
	$.ajax({
		url:url+"getIscsAcchpRelUICurves.htm",
		type:"post",
		//data:"equ_num="+equ_num,
		data:"equ_num="+equNum,
		dataType:"json",
...
function xiangqing(){
	var equNum=$("#equNum").val();
		...
		layer.open({
			  ...
			  shadeClose: false,
			  area: ['770px','450px'],
			  content: "<%=basePath%>/commens/zhjk/sb/xq/jlcdz.htm?equNum="+equNum   //iframe的url
		});
