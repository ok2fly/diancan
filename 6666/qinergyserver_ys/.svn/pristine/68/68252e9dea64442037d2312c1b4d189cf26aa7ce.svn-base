...
<form  action="" id="uploadModel"  enctype="multipart/form-data" >
	<input type="hidden" value="${id}" id="id"  name="id">
 	<table cellpadding="0"   border="0" width="100%" class="poptable">
		...
		<tr> 
			<th>公司项目名称</th>
			<td><input type="text" name="comPjtNam" id="comPjtNam" placeholder="请输入公司项目名称" class="popinput quinputcol "></td>
			<th>附件</th>
...
$(function(){
	...
	$.ajax({                
		type : "post",
		url: url + "getBasComInfo.htm",
		data : "id="+id,
		async : false,
		success: function(data){                          
			if('USR000' == data.resultcode){
			 	var d = data.data.basCILst[0];
			 	$("#comPjtNam").val(d.com_pjt_nam);
